@extends('layouts.app')
@section('title', 'Laporan Kelas Asuhan — EcoBank SMKN 2 Indramayu')
...
<div class="animate-fade-in" style="margin-bottom:var(--s-32);">
    ...
    <div class="greeting-row">
        ...
        <div>
            <div class="greeting-name">Laporan Transaksi Kelas Asuhan</div>
            <div class="greeting-meta">Filter dan analisis data transaksi siswa untuk kelas binaan Anda</div>
        </div>
    </div>
    ...
    <div class="card" style="margin-top: var(--s-20); border-top: 5px solid var(--accent);">
        <h3 style="font-family:var(--font-display); font-size:16px; font-weight:800; color:var(--primary); margin-bottom:var(--s-16); display:flex; align-items:center; gap:8px;">
            <i class="bi bi-sliders" style="color:var(--accent);"></i> Filter Parameter Transaksi Kelas
        </h3>

        <form action="{{ route('walikelas.reports') }}" method="GET" id="report-filter-form">
            <div style="display:grid; grid-template-columns: repeat(auto-fit, minmax(220px, 1fr)); gap: var(--s-12);">
                ...
                {{-- Class (restricted dropdown to teacher's classes) --}}
                <div class="form-group">
            ...
            <div style="margin-top:var(--s-16); display:flex; gap:var(--s-8); justify-content:flex-end;">
                <a href="{{ route('walikelas.reports') }}" class="btn btn-outline" style="border: 1.5px solid var(--outline-variant); color: var(--on-background); font-weight:700; text-decoration:none; padding: 6px 16px; border-radius:var(--r-sm); font-size:13px; display:inline-flex; align-items:center; gap:4px;">
    ...
    @if(count($transactions) > 0)
        <div class="operator-stats-grid" style="grid-template-columns: repeat(auto-fit, minmax(200px, 1fr)); margin-top: var(--s-20);">
            <div class="stat-item-box">
                ...
                <div class="stat-item-label">Total Berat Sampah Kelas</div>
            ...
            <div class="stat-item-box">
                ...
                <div class="stat-item-label">Total Nilai Tabungan</div>
            ...
            <div class="stat-item-box">
                ...
                <div class="stat-item-label">Total Uang Ditarik</div>
    ...
    <div class="card" style="margin-top:var(--s-20);">
        <div class="flex-between" style="margin-bottom:var(--s-16);">
            <h2 style="font-size:15px;font-weight:800;color:var(--on-surface);display:flex;align-items:center;gap:var(--s-8);">
                <i class="bi bi-list-task" style="color:var(--primary);"></i> Transaksi Siswa Kelas Anda
            </h2>
            
            {{-- Export Buttons --}}
            @if(count($transactions) > 0)
                <div style="display:flex; gap:8px;">
                    <a href="{{ route('walikelas.reports.excel', request()->all()) }}" class="btn btn-outline" style="border:1.5px solid #22c55e; color:#15803d; font-weight:700; text-decoration:none; padding:4px 10px; font-size:12px; border-radius:var(--r-sm); display:inline-flex; align-items:center; gap:4px;">
                        <i class="bi bi-file-earmark-excel"></i> Excel
                    </a>
                    <a href="{{ route('walikelas.reports.pdf', request()->all()) }}" class="btn btn-outline" style="border:1.5px solid #ef4444; color:#b91c1c; font-weight:700; text-decoration:none; padding:4px 10px; font-size:12px; border-radius:var(--r-sm); display:inline-flex; align-items:center; gap:4px;">
                    ...
                    @empty
                        <tr>
                            <td colspan="11" style="text-align:center; padding:var(--s-32); color:var(--on-surface-variant);">
                                ...
                                Tidak ditemukan data transaksi kelas asuhan binaan Anda.
                            </td>
                        </tr>
                    @endforelse
                </tbody>
