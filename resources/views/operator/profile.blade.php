...
<div class="animate-fade-in" style="margin-bottom:var(--s-32);">
    ...
    <div style="margin-bottom: var(--s-24);">
        <a href="{{ route('operator.dashboard') }}" class="back-link" style="display:inline-flex; align-items:center; gap:6px; font-weight:700; color:var(--primary); text-decoration:none; margin-bottom: 8px;">
        ...
        <div class="dashboard-column-left">
            <div class="card" style="padding:0; overflow:hidden; border-top: 5px solid var(--primary);">
                <div class="profile-hero" style="background:linear-gradient(180deg, var(--primary-container) 0%, var(--surface) 100%); padding: var(--s-32) var(--s-24); text-align:center;">
                    ...
                    <h3 style="font-family:var(--font-display); font-size:20px; font-weight:800; color:var(--on-surface);">{{ $operator->name }}</h3>
                    <div style="margin-top:6px;">
                        <span class="badge badge-primary" style="background:var(--primary); color:white; padding: 4px 12px; border-radius:var(--r-full); font-size:11px; font-weight:700;">Operator Bank Sampah</span>
                    </div>
                </div>
                ...
                <div style="padding: var(--s-24) var(--s-28); border-top: 1px dashed var(--outline-variant);">
                    ...
                    <div class="info-grid" style="display:flex; flex-direction:column; gap:var(--s-12);">
                        <div style="display:flex; justify-content:space-between; font-size:13.5px; border-bottom:1px solid var(--outline-variant); padding-bottom:var(--s-8);">
                            ...
                            <strong style="color:var(--on-surface);">{{ $operator->email }}</strong>
                        </div>
                        <div style="display:flex; justify-content:space-between; font-size:13.5px; border-bottom:1px solid var(--outline-variant); padding-bottom:var(--s-8);">
                            <span style="color:var(--on-surface-variant); font-weight:600;"><i class="bi bi-telephone" style="margin-right:6px;"></i> Telepon</span>
                            <strong style="color:var(--on-surface);">{{ $operator->phone ?? '—' }}</strong>
                        </div>
                        <div style="display:flex; justify-content:space-between; font-size:13.5px;">
                            <span style="color:var(--on-surface-variant); font-weight:600;"><i class="bi bi-shield-check" style="margin-right:6px;"></i> Wewenang</span>
                            <strong style="color:var(--primary); font-size:12px;">Catat Setoran &amp; Konfirmasi</strong>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="card" style="padding: var(--s-32) var(--s-28); border-top: 5px solid var(--accent);">
                ...
                <form action="{{ route('operator.profile.update') }}" method="POST">
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" id="name" name="name" class="form-control" value="{{ old('name', $operator->name) }}" required>
                        @error('name')
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" id="phone" name="phone" class="form-control" value="{{ old('phone', $operator->phone) }}" placeholder="Contoh: 081234567890">
                        @error('phone')
                            <span class="form-error" style="color:var(--danger); font-size:12px; margin-top:6px; display:block;"><i class="bi bi-exclamation-circle"></i> {{ $message }}</span>
                        @enderror
                    </div>
