@extends('layouts.app')
@section('title', 'Edit Kategori Sampah — EcoBank SMKN 2 Indramayu')
...
<div class="animate-fade-in" style="margin-bottom:var(--s-32);">
    ...
    <div style="margin-bottom: var(--s-24);">
        ...
        <h2 style="font-family:var(--font-display); font-size:26px; font-weight:800; color:var(--primary); margin-top:4px;">Edit Kategori Sampah</h2>
    </div>
    ...
    <div class="category-settings-grid">
            ...
            <div style="background:var(--surface-dim); border: 2px solid var(--outline-variant); border-radius:var(--r-lg); padding: var(--s-24) var(--s-16); max-width: 240px; margin: 0 auto; box-shadow: var(--shadow-sm);">
                ...
                <div id="preview-icon-box" style="width:64px; height:64px; border-radius:50%; background:var(--surface); margin:0 auto var(--s-12) auto; display:flex; align-items:center; justify-content:center; box-shadow:var(--shadow-sm); border:1px solid var(--outline-variant); overflow:hidden;">
                    @if(Str::startsWith($category->icon, '/uploads/') || Str::startsWith($category->icon, 'http'))
                        <img id="preview-icon-img" src="{{ $category->icon }}" style="width:100%; height:100%; object-fit:cover;">
                        <span id="preview-icon-emoji" style="font-size:28px; display:none;">🥤</span>
                    @else
                        <img id="preview-icon-img" style="width:100%; height:100%; object-fit:cover; display:none;">
                        <span id="preview-icon-emoji" style="font-size:28px;">{{ $category->icon }}</span>
                    @endif
                </div>
                ...
                <div style="margin-top:var(--s-16); border-top:1px dashed var(--outline-variant); padding-top:var(--s-12); display:grid; grid-template-columns:1fr 1fr; gap:8px;">
                    ...
                    <div>
                        ...
                        <span style="font-family:var(--font-mono); font-weight:800; font-size:13px; color:var(--accent);" id="preview-points">
                            {{ number_format($category->points_per_kg, 0, ',', '.') }} Pts
                        </span>
                    </div>
                </div>
            </div>
        ...
        <div class="card" style="border-top: 5px solid var(--primary); padding: var(--s-28);">
            <form action="{{ route('manajer.categories.update', $category->id) }}" method="POST" enctype="multipart/form-data">
                @csrf
                ...
                <div style="display:grid; grid-template-columns: 1fr 1fr; gap:var(--s-16);">
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" id="name" name="name" class="form-control" value="{{ old('name', $category->name) }}" required placeholder="Contoh: Botol Plastik PET" oninput="updatePreviewName(this.value)">
                        @error('name')
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" id="key" name="key" class="form-control" value="{{ old('key', $category->key) }}" required placeholder="Contoh: plastik_botol" oninput="updatePreviewKey(this.value)">
                        @error('key')
                            <span class="form-error" style="color:var(--danger); font-size:12px; margin-top:4px; display:block;">{{ $message }}</span>
                        @enderror
                    </div>
                ...
                <div style="display:grid; grid-template-columns: 1fr 1fr; gap:var(--s-16); margin-top:var(--s-16);">
                    ...
                    <div class="form-group">
                        ...
                        <input type="number" min="0" id="price_per_kg" name="price_per_kg" class="form-control" value="{{ old('price_per_kg', $category->price_per_kg) }}" required style="font-family:var(--font-mono);" oninput="updatePreviewPrice(this.value)">
                        @error('price_per_kg')
                            <span class="form-error" style="color:var(--danger); font-size:12px; margin-top:4px; display:block;">{{ $message }}</span>
                        @enderror
                    </div>
                    ...
                    <div class="form-group">
                        ...
                        <input type="number" min="0" id="points_per_kg" name="points_per_kg" class="form-control" value="{{ old('points_per_kg', $category->points_per_kg) }}" required style="font-family:var(--font-mono);" oninput="updatePreviewPoints(this.value)">
                        @error('points_per_kg')
                            <span class="form-error" style="color:var(--danger); font-size:12px; margin-top:4px; display:block;">{{ $message }}</span>
                        @enderror
                    </div>
                ...
                <div class="form-group" style="margin-top:var(--s-16); border:1px dashed var(--outline-variant); padding:var(--s-16); border-radius:var(--r-md); background:var(--surface-dim);">
                    <label for="icon_image" class="form-label" style="font-weight:700; color:var(--primary); display:flex; align-items:center; gap:6px;">
                        <i class="bi bi-image" style="font-size:16px;"></i> Unggah Gambar Ikon Baru (Direkomendasikan)
                    </label>
                ...
                <div class="form-group" style="margin-top:var(--s-16);">
                    ...
                    <input type="text" id="icon" name="icon" class="form-control" value="{{ old('icon', Str::startsWith($category->icon, '/uploads/') ? '' : $category->icon) }}" placeholder="Contoh: 🥤, 📦, 🥫, atau leaf" oninput="updatePreviewEmoji(this.value)">
